...
template<typename T>
inline FloatRect frect(T a, T b, T c, T d) {
	return FloatRect({static_cast<float>(a), static_cast<float>(b)}, {static_cast<float>(c), static_cast<float>(d)});
}
...
void Terminal::forceRedraw(Font &font, vector<Vertex> &target, Color bgColor) {
	...
	if (bgColor != Color::Black)
		pushVertexColor(target, frect(0, 0, (cols + 1) * cellSize.x, (rows + 1) * cellSize.y), bgColor);
	...
	for (int i = 0; i < rows; i++)
		for (int j = 0; j < cols; j++) {
			...
			FloatRect cellRect({static_cast<float>(j * cellSize.x), static_cast<float>(i * cellSize.y)}, {static_cast<float>(cellSize.x * cell.width), static_cast<float>(cellSize.y)});
			...
			if (cursorVisible && curpos.row == i - scrollbackOffset && curpos.col == j) {
				...
				switch (cursorShape) {
					...
					case VTERM_PROP_CURSORSHAPE_BAR_LEFT:
						pushVertexColor(target, frect(cellRect.left, cellRect.top, thick, cellRect.height), reverseOf(bg));
						break;
					case VTERM_PROP_CURSORSHAPE_UNDERLINE:
						pushVertexColor(target, frect(cellRect.left, cellRect.top + cellRect.height - 1 - thick, cellRect.width, thick), reverseOf(bg));
						break;
				}
			}
			...
			if (cell.attrs.underline) {
				float thick = font.getUnderlineThickness(charSize);
				pushVertexColor(target, frect(cellRect.left, cellRect.top + cellRect.height - 1 - thick, cellRect.width, thick), fg);
			}
			if (cell.attrs.strike) { // I'm too lazy to somehow get the strikeline position
				float thick = font.getUnderlineThickness(charSize);
				pushVertexColor(target, frect(cellRect.left, roundf(cellRect.top + cellRect.height * 0.6f - thick / 2.0f), cellRect.width, thick), fg);
			}
