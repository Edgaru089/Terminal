...
Terminal::Terminal(
	...
	: frontend(frontend), cols(cols), rows(rows), cellSize(cellSize), charSize(charSize), hasBold(useBold), scrollbackMaxLength(scrollbackMaxLength), winTitle("Terminal"), charTopOffset(-4096), cursorVisible(true) {
	running = true;
...
sf::Vector2u Terminal::getIMEPosition() {
	VTermPos curpos;
	vterm_state_get_cursorpos(vterm_obtain_state(term), &curpos);
	return sf::Vector2u(
		static_cast<unsigned int>(curpos.col * cellSize.x),
		static_cast<unsigned int>(curpos.row * cellSize.y));
}
