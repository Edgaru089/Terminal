...
void Terminal::processEvent(RenderWindow &win, Event e) {
	switch (e.type) {
		case Event::TextEntered: {
			...
			fprintf(stderr, "Main: Event::TextEntered, keycode=%d(%c), Ctrl:%s, Shift:%s, Alt:%s\n", (int)e.text.unicode, (char)e.text.unicode, (mod & VTERM_MOD_CTRL) ? "Yes" : "No", (mod & VTERM_MOD_SHIFT) ? "Yes" : "No", (mod & VTERM_MOD_ALT) ? "Yes" : "No");
		...
		case Event::MouseWheelScrolled: {
			if (mouseState != VTERM_PROP_MOUSE_NONE) {
				vterm_mouse_button(term, ((e.mouseWheelScroll.delta > 0) ? 4 : 5), true, getModifier());
				vterm_mouse_button(term, ((e.mouseWheelScroll.delta > 0) ? 4 : 5), false, getModifier());
			} else if (altScreen) {
